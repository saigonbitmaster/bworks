...
exports.hashFileName = (options, fileName) => {
  let fileExt = fileName.split('.').pop();
  let current = moment();
  let result = crypto.createHash('sha1').update(`${current.utc()}${fileName}`).digest('base64');
  // Replace slash with another random character
  result = result.replace(/\//g, randomChar()); 
  result = `${current.format('YYMMDDHH')}${result.substr(0, 6)}.${fileExt}`;
...
exports.upload = function(provider, req, res, options, cb) {
  ...
  form.handlePart = function(part) {
    ...
    var file = {
      container: container,
      name: exports.hashFileName(options, part.filename),
      type: part.mime,
      field: part.name,
    };
...
exports.download = function(provider, req, res, container, file, cb) {
  ...
  function getStream(provider, container, file, options) {
    var params = { container, remote: file, ...options };
