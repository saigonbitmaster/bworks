...
RUN apk add --update git openssh-client python3 make g++
...
RUN rm -rf $(find $PWD/packages -maxdepth 1 -mindepth 1 -type d -exec ls -d "{}" + | grep -v "water-api" | grep -v "ra-loopback3")
...
RUN mv packages/water-api/server/datasources.docker.js packages/water-api/server/datasources.production.js
...
RUN mkdir -p /home/node/app/packages/water-api/temp
RUN chown -R node:node /home/node/app/packages/water-api/temp
RUN chown -R node:node /home/node/app/packages/water-api/tempSpreadSheet
