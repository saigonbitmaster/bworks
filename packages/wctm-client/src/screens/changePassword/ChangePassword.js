...
const validatePassword = (translate, values) => {
  ...
  if (!mediumRegex.test(tmpNew)) {
    errors.newPassword = [translate('resources.clientusers.messages.errorNewPassword')];
  }
  let tmpConfirm = confirmPassword ? confirmPassword.toLowerCase() : '';
  if (tmpNew !== tmpConfirm) {
    errors.confirmPassword = [translate('resources.clientusers.messages.errorConfirmPassword')];
  }
  return errors;
};
class ChangePassword extends Component {
  ...
  save = (record, redirect, defaultOnSuccess, defaultOnFailure) => {
    ...
    this.props
      .dataProvider(
        ...
        {
          notification: { body: 'resources.clientusers.messages.passwordUpdated', level: 'info' },
          // reset form
  ...
  render() {
    ...
    return (
      <Create save={this.save} resource="clientusers" {...rest} hasList={false}>
        <FlexForm
          style={{ flexGrow: 1 }}
          spacing={2}
