...
function Printer(options = {}) {
  const { minFragment = 512, model = null } = options;
...
Printer.prototype.tableCustom = function(data, options = {}) {
  ...
  // Write the line
  this.buffer.write(iconv.encode(lineStr + _.EOL, options.encoding || this.encoding));
...
Printer.prototype.image = function(image, density) {
  ...
  // this.lineSpace();
  // buffs.push(self.buffer.flush());
  return buffs.map(buf => buf.toString('base64'));
};
