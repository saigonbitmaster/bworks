...
module.exports = ClientUser => {
  ...
  ClientUser.requestToCreateNewAccount = async (username, email, req) => {
    // Use `code` to query matched client
    // Compare given `email` to one in the fetch client
    // If they are matched, put the approved data on hold with Redis
    // If they aren't, reject right away
    // If there is none in the fetched client, put the unapproved data on hold on Redis
    const code = username;
    ...
    if (client) {
      ...
      if (clientUser) {
        ...
        const mailPayload = {
          from: SENDER_EMAIL,
          to: email,
          subject: 'Đặt mật khẩu cho tài khoản mới',
