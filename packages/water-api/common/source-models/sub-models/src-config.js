...
module.exports = function(SrcConfig) {
  SrcConfig.remoteMethod('getUnusedChoices', {
    accepts: [],
    returns: { root: true, type: ['object'] },
    http: { verb: 'get' },
  });

  SrcConfig.remoteMethod('getAttributeChoices', {
    accepts: [{ arg: 'id', type: 'any' }],
    returns: { root: true, type: ['object'] },
    http: { verb: 'get' },
  });

  SrcConfig.observe('before save', async function checkData(ctx) {
  ...
  SrcConfig.getUnusedChoices = async () => {
    let result = [];
    let allChoices = SrcConfig.definition.properties.id.enum;
    // get current list
    let currentList = (await SrcConfig.find({ fields: { id: true } })) || [];
  ...
  SrcConfig.getAttributeChoices = async id => {
    let allChoices = SrcConfig.definition.properties.id.enum;
    let result = {};
    // get type by id
  ...
  SrcConfig.getConfigByKey = async key => {
    let config = await SrcConfig.findById(key);
    if (!config) {
      let defaultConfig = SrcConfig.app.get(camelCase(key));
      if (defaultConfig) {
