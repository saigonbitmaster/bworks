...
class ConfigurationList extends Component {
  ...
  translateId = element => {
    let id = element.id;
    return this.props.translate(`resources.ctmconfigs.fields.ids.${id}`);
  };
  translateSide = element => {
    let side = element.side;
    return this.props.translate(`resources.ctmconfigs.fields.sides.${side}`);
  };
  render() {
    return (
      <List
        title={this.props.translate('resources.ctmconfigs.listTitle')}
        {...this.props}
        resource="ctmconfigs"
        filters={<ConfigurationFilter translate={this.props.translate} />}
        permissionCreateDefault={{ name: 'Configuration', action: 'create' }}
        permissionDeleteDefault={{ name: 'Configuration', action: 'delete' }}
      >
        <Responsive
          medium={
            <Datagrid>
              <ReferenceField reference="ctmconfigs" source="id">
              ...
              <SFEditButton permission={{ name: 'Configuration', action: 'edit' }} />
            </Datagrid>
          }
          small={
            <Datagrid>
