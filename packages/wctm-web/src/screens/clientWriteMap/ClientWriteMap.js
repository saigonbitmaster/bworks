...
class ClientWriteMap extends Component {
  ...
  state = {
    ...
    data: [],
    filteredData: [],
    writeStatus: config.client.writeChoices,
    chosenWriteStatus: true,
    centerPointGeo: {},
  ...
  componentDidMount() {
    ...
    dispatch(change('formFilterClientWriteMap', 'status', this.state.chosenWriteStatus));
  }
  ...
  getData = (time, provinceId, districtId, wardId, { latitude, longitude }) => {
    ...
    dataProvider(CUSTOM, 'clients', {
      ...
      .then(res => {
        ...
        if (res && res.data) {
          const data = res.data;
          const filteredData = res.data.filter(({ status }) => status === this.state.chosenWriteStatus);
  ...
  handleStatusChange = (_, value) => {
    ...
    const preTextFilteredData = filteredData;
    this.setState({ filteredData, preTextFilteredData, chosenWriteStatus: value }, () => {
      if (has(this.state, 'filteredText')) {
        this.handleFilterClientByName(null, this.state.filteredText);
      }
    });
  ...
  render() {
    ...
    const { filteredData, writeStatus, centerPointGeo, zoomLevel, statisticClient, chosenWriteStatus } = this.state;
    return (
      <CustomPage rawTitle={title} header card>
        <Grid container>
            ...
            <Card className={classes.condition}>
              <CardContent className={classes.condition}>
                <FlexFormFilter
                  ...
                  formName={'formFilterClientWriteMap'}
                  resource="clients"
                ...
                >
                  <Grid middle container spacing={2}>
                    ...
                    />
                    <SelectInput source="status" choices={writeStatus} onChange={this.handleStatusChange} />
                    <ReferenceInput
          ...
          <Grid item xs={12} style={{ display: 'flex' }}>
            <ShowMap
              selectedStatus={chosenWriteStatus}
...
export default enhance(ClientWriteMap);
