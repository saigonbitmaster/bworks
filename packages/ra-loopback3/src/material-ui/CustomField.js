...
/**
 * The `CustomField` is a convenience wrapper for the most common cases (80%).
 ...
 * };
 *
 * return <CustomField id="time" type="time" inputProps={inputProps} />;
 * ```
 *
 * For advanced cases, please look at the source of CustomField by clicking on the
...
function CustomField(props) {
  ...
  warning(
    ...
    'Material-UI: `children` must be passed when using the `CustomField` component with `select`.',
  );
  ...
  const InputElement = (
    <CustomInput
      ...
      rowsMax={rowsMax}
      type={type}
      value={value || lodash.get(InputProps.inputProps, 'defaultValue')}
      id={id}
      inputRef={inputRef}
      onBlur={onBlur}
      onChange={onChange}
