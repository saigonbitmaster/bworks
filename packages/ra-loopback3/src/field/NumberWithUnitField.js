...
export const NumberWithUnitField = ({
  ...
  let fixUnit = unitKey ? get(record, unitKey) : unit;
  ...
  return (
    <span className={classnames(classes.input, className)} {...sanitizeRestProps(rest)}>
      {value.toLocaleString(locales, options)} {translateUnit ? translate(fixUnit) : fixUnit}
    </span>
  );
};
...
NumberWithUnitField.propTypes = {
  ...
  translateUnit: PropTypes.bool,
};

const FixLocateNumber = compose(withLocales, pure, withStyles(styles))(NumberWithUnitField);
