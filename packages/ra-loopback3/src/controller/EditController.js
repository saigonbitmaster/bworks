...
import { crudGetOne, crudUpdate, startUndoable, translate } from 'react-admin';
import { REDUX_FORM_NAME } from 'ra-core/lib/form';
/**
...
export class EditController extends Component {
  ...
  save = (data, redirect) => {
    const { undoable = true, startUndoable, dispatchCrudUpdate, save } = this.props;
    if (save) {
      save(
        data,
        redirect,
      ...
      if (undoable) {
        ...
        dispatchCrudUpdate(this.props.resource, this.props.id, data, this.props.record, this.props.basePath, redirect);
      }
    }
  };
