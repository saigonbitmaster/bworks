...
import { crudGetManyAccumulate as crudGetManyAccumulateAction, getReferencesByIds } from 'react-admin';
...
export class ReferenceArrayFieldController extends Component {
  ...
  render() {
    const { resource, reference, data, ids, children, basePath, allowEmpty } = this.props;
    const referenceBasePath = basePath.replace(resource, reference); // FIXME obviously very weak
    let fixData = allowEmpty ? data || {} : data;
    return children({
      isLoading: ids.length !== 0 && !fixData,
      ids,
      data: fixData,
      referenceBasePath,
      currentSort: {},
