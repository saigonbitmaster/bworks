...
import { addField, translate, ReferenceInputController } from 'react-admin';
 ...
 * </ReferenceInput>
 */
export const ReferenceInput = ({ children, filter, ...props }) => {
  if (React.Children.count(children) !== 1) {
    throw new Error('<ReferenceInput> only accepts a single child');
  }
  // eslint-disable-next-line react/prop-types
  const fixFilter = props.disabled ? { id: 'disabled' } : filter;

  return (
    <ReferenceInputController {...props} filter={fixFilter}>
      {controllerProps => <ReferenceInputView {...props} filter={fixFilter} {...{ children, ...controllerProps }} />}
