...
import { SimpleForm, SelectInput, CUSTOM, ReferenceInput } from 'ra-loopback3';
...
class ClientGroupedByGeo extends Component {
  static propTypes = {
  ...
  getData = (provinceId, districtId, wardId) => {
    const { dataProvider } = this.props;
    dataProvider(CUSTOM, 'Clients', {
      subUrl: 'getClientGroupedByGeo',
      method: 'get',
      fullUrl: true,
      query: { provinceId, districtId, wardId },
    }).then(({ data }) => {
      this.setState({ data });
    });
  };
  ...
  handleGeoChange = (changedProvinceId, changedDistrictId, changedWardId) => {
    ...
    if (changedProvinceId !== null) {
      if (changedProvinceId !== provinceId && (!isUndefined(districtId) || !isUndefined(wardId))) {
        change('client-grouped-by-geo-form', 'districtId', '');
        change('client-grouped-by-geo-form', 'wardId', '');
        newDistrictId = '';
        newWardId = '';
      }
    ...
    return (
      <Paper className={classes.paper}>
        <Typography variant="h6" gutterBottom className={classes.header}>
          {translate('resources.clients.statistic.clientGroupedByGeo')}
        </Typography>
        <div className={classes.geoButtons}>
          <SimpleForm form={'client-grouped-by-geo-form'} toolbar={null}>
            <ReferenceInput
        ...
        {data.length > 0 ? (
          <ResponsiveContainer width="95%" height="60%" className={classes.treemapContainer}>
            ...
            >
              <Tooltip formatter={(value, _, { payload: { name } }) => [`${format.number(value)} đơn vị`, name]} />
            </Treemap>
          </ResponsiveContainer>
        ) : (
          <div style={{ paddingLeft: '16px' }}>{'Không có dữ liệu'}</div>
        )}
      </Paper>
    );
  }
...
export default compose(
  ...
  connect(
    state => ({
      formValues: formValueSelector('client-grouped-by-geo-form')(state, 'provinceId', 'districtId', 'wardId'),
    }),
    { change },
  ),
)(ClientGroupedByGeo);
