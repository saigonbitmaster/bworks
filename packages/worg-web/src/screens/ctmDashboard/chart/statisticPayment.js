      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={12}>
          <Paper className={classes.paper}>
            <Header title="resources.clients.statistic.statisticPayment" />
            <Grid container>
              <Grid item xs={12} sm={12}>
                <FilterFromToTime
                  formName="formFilterStatisticPayment"
              ...
              <Grid item xs={12} sm={12}>
                <ResponsiveContainer width="100%" aspect={2}>
                  <ComposedChart data={dataCollect} margin={{ top: 20, right: 10, left: 30, bottom: 5 }}>
                    ...
                    <Bar
                      dataKey="countClientPaid"
                      stackId="a"
                      fill={'#4dbd74'}
                      name={translate('resources.clients.statistic.clientPaid')}
                      formatter={this.format}
                    />
                    <Bar
                      dataKey="countClientUnPaid"
                      stackId="b"
                      fill={'#ff5454'}
                      name={translate('resources.clients.statistic.clientUnPaid')}
                      formatter={this.format}
                    />
                    <Bar
                      dataKey="sumClient"
...
StatisticPayment.propTypes = {
  theme: PropTypes.object,
  translate: PropTypes.func,
  classes: PropTypes.object,
  dataProvider: PropTypes.func,
};
StatisticPayment.detaultProps = {
  hasList: true,
  hasShow: true,
  hasCreate: false,
  hasEdit: false,
...
export default enhance(StatisticPayment);
