...
class StatisticFlowLogger extends Component {
  ...
  onChangeFilterChart = filter => {
    ...
    this.setState({ filterStatistic: filter.time });
    let { type, from, to } = filter.time;
    if (type === 'hour') {
      this.loadDataFlowPressure(from);
    }
    this.loadDataSumFlow(type, from, to);
  ...
  onChangeFilterAccumulate = val => {
    ...
    let tmp = moment(val.dayAccumulateFlow).add(hourCurrent, 'hour'); // 00 + hourCurrent
    this.loadDataFlowAccumulate(tmp);
  };
  render() {
    ...
    let textResultAccumlate = translate('generic.messages.hasNotDataLogger');
    if (dataFlowAccumalate && dataFlowAccumalate.logTime && dataFlowAccumalate.maxFlow) {
      textResultAccumlate = translate('generic.messages.notifyResultAccumulateFlow', {
      ...
      <CustomPage title={nameLogger}>
        <Grid container spacing={2 * 2}>
          <Grid item xs={12} sm={12}>
            <CustomPage
              title={translate('generic.accumulateFlowDatalogger', { val: nameLogger })}
              card
              header
            ...
            >
              <Grid container>
                <Grid item xs={12} sm={12}>
                  ...
                  >
                    <DateInput source="dayAccumulateFlow" label={translate('generic.typeTime.day')} />
                  </FlexFormFilter>
                </Grid>
          ...
          <Grid item xs={12} sm={12}>
            <CustomPage title={translate('generic.chartFlowPressureDatalogger', { val: nameLogger })} card header>
              <Grid container>
                <Grid item xs={12} sm={12}>
...
const enhance = compose(translate, withDataProvider, withStyles(styles), withTheme, connect(null, { goBack }));
