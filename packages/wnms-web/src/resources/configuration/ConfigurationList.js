...
class ConfigurationList extends Component {
  ...
  translateId = element => {
    let id = element.id;
    return this.props.translate(`resources.nmsconfigs.fields.ids.${id}`);
  };
  translateSide = element => {
    let side = element.side;
    return this.props.translate(`resources.nmsconfigs.fields.sides.${side}`);
  };
  render() {
    return (
      <List
        title={this.props.translate('resources.nmsconfigs.listTitle')}
        {...this.props}
        resource="nmsconfigs"
      ...
      >
        <Responsive
          medium={
            <Datagrid>
              <ReferenceField reference="nmsconfigs" source="id">
