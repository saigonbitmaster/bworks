...
// import LoggerChartHour from './LoggerChartHour';
// import LoggerChartDay from './LoggerChartDay';
...
class ChartSymbol extends Component {
  ...
  queryReport = filter => {
    ...
    //  this.setState({ selectedParamSymbol: filter.waterParameter });
  ...
  statistic = async filter => {
    ...
    let res = await dataProvider(CUSTOM, 'materialuses', {
      method: 'get',
      subUrl: 'nmsGetDataChartElectric',
      ...
      <CustomPage title={title}>
        <Grid container spacing={2}>
          <Grid item xs={12} sm={12}>
            <Filter
              formName={'nms-filter-electric-chart'}
